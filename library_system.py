'''一个图书管理系统，能够实现增加书籍，删除书籍，修改书籍和查看图书详情，基于mysql数据库和wxPython'''
#主界面代码
import wx        # 面板
from book import *     #导入book._all_中的全部元素
from dbhelper import *
import tkinter
from tkinter import messagebox  #将tkinter.messagebox导入到当前环境，messagebox可直接执行
__metaclass__ = type
#增加文件#
...
class LibraryFrame(wx.Frame):
	def __init__(self, parent, title):
		...
		wx.Frame.__init__(self, parent, title=title, size=(400, 340))
		...
		#生成一个列表
		self.list = wx.ListCtrl(self, -1, size = (400,200), style = wx.LC_REPORT | wx.LC_HRULES | wx.LC_VRULES) #| wx.LC_SINGLE_SEL
		...
		self.panel = wx.Panel(self, pos = (0, 200), size = (400, 200))
		#定义一组按钮
		...
		dingyue_button = wx.Button(self.panel, label = "借阅", pos = (110,60), size= (60,30))
		quxiao_button = wx.Button(self.panel, label = "取消借阅", pos = (210,60), size= (60,30))
		update_button = wx.Button(self.panel, label = "修改", pos = (210, 15), size = (60, 30)) #, size = (75, 30)
		query_button = wx.Button(self.panel, label = "查看", pos = (310, 15), size = (60, 30))
		...
		self.Bind(wx.EVT_BUTTON, self.dingyue,dingyue_button)
		self.Bind(wx.EVT_BUTTON, self.quxiao, quxiao_button)
	...
	def addBook(self, evt):
		...
		add_f.Show(True)
	def dingyue(self, evt):
		messagebox.showinfo("借阅","借阅成功")

	def quxiao(self, evt):
		print("222")
		messagebox.showinfo("取消借阅","取消订阅成功")
